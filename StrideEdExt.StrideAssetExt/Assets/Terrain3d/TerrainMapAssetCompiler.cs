...
public class TerrainMapAssetCompiler : AssetCompilerBase
{
    ...
    protected override void Prepare(AssetCompilerContext context, AssetItem assetItem, string targetUrlInStorage, AssetCompilerResult result)
    {
        ...
        result.BuildSteps.Add(new TerrainMapAssetCommand(targetUrlInStorage, asset, assetItem.Package, assetFilePath, texImportParams));
    }
    ...
    private class TerrainMapAssetCommand : AssetCommand<TerrainMapAsset>
    {
        ...
        public TerrainMapAssetCommand(string url, TerrainMapAsset parameters, IAssetFinder assetFinder, UFile assetFilePath, TerrainTextureImportParameters textureImportParams)
        ...
        protected override Task<ResultStatus> DoCommandOverride(ICommandContext commandContext)
        {
            ...
            lock (terrainMapAsset)     // Lock to prevent TerrainMapAssetViewModel from modifying it
            {
                terrainMapAsset.OnAssetLoaded(_assetFullFilePath, logger);
                var terrainMap = terrainMapAsset.ToTerrainMap(logger);
                ...
                if (terrainMapAsset.MaterialIndexMapData is Array2d<byte> materialIndexMapData)
                {
                    try
                    {
                        ...
                        for (int y = 0; y < pixelBuffer.Height; y++)
                        {
                            for (int x = 0; x < pixelBuffer.Width; x++)
                            {
                                ...
                                byte byteValue = materialIndexMapData[x, y];
                        ...
                        logger.Info($"{Url}: Processing Terrain Material Index Map Texture: Writing to {texImportParams.OutputUrl}");
                    ...
                    {
                        logger.Error($"{Url}: Failed to process Terrain Material.", ex);
                ...
                if (terrainMap.TerrainMaterial is TerrainMaterial terrainMaterial)
                {
                    ...
                    if (terrainMaterialAttachedRef is not null)
                    {
                        ...
                        {
                            var terrainMaterialAssetId = terrainMaterialAttachedRef.Id;
                            terrainMap.TerrainMaterial = AttachedReferenceManager.CreateProxyObject<TerrainMaterial>(terrainMaterialAssetId, terrainMaterialAttachedRef.Url);
                        }
                    }
                }
        ...
        {
            bool useSeparateDataContainer = TextureHelper.ShouldUseDataContainer(isStreamable: false, texImage.Dimension);
