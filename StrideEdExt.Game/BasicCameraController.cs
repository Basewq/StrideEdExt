...
namespace SceneEditorExtensionExample
{
    ...
    public class BasicCameraController : SyncScript
    {
        ...
        public bool IsTerrainScene { get; set; }
        public UrlReference<Scene>? NextScene { get; set; }
        ...
        public override void Update()
        {
            ...
            if (_showHints)
            {
                ...
                PrintLine("Change Scene with [F2]", ref nextPrintLinePos);
                ...
                PrintLine("", ref nextPrintLinePos);
                PrintLine($"CamPos: {ToDisplayString(Entity.Transform.Position)}", ref nextPrintLinePos);
            }
            ...
            static string ToDisplayString(Vector3 vec3)
            {
                return $"X:{vec3.X:n4} Y:{vec3.Y:n4} Z:{vec3.Z:n4}";
            }

            static Ray CreateMouseRay(CameraComponent cameraComponent, Vector2 normalisedScreenPosition)
            {
                Matrix.Invert(ref cameraComponent.ViewProjectionMatrix, out Matrix invViewProj);

                var sPos = new Vector3(
                    x: normalisedScreenPosition.X * 2f - 1f,
                    y: 1f - normalisedScreenPosition.Y * 2f,
                    z: 0f
                );

                Vector3.Transform(ref sPos, ref invViewProj, out Vector4 vectorNear);
                Vector4.Divide(ref vectorNear, vectorNear.W, out vectorNear);

                sPos.Z = 1f;
                Vector3.Transform(ref sPos, ref invViewProj, out Vector4 vectorFar);
                Vector4.Divide(ref vectorFar, vectorFar.W, out vectorFar);
                var rayDir = (Vector3)(vectorFar - vectorNear);
                rayDir.Normalize();
                return new Ray((Vector3)vectorNear, rayDir);
            }
        }

        private void ProcessInput()
        {
            if (Input.HasKeyboard && Input.IsKeyPressed(Keys.F2) && NextScene is not null)
            {
                var scene = Content.Load(NextScene);
