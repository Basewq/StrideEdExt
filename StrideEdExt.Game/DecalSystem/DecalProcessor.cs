...
namespace StrideEdExt.DecalSystem;
...
class DecalProcessor : EntityProcessor<DecalComponent, DecalProcessor.AssociatedData>, IEntityComponentRenderProcessor
{
    ...
    protected override void OnEntityComponentAdding(Entity entity, [NotNull] DecalComponent component, [NotNull] AssociatedData data)
    {
        ...
        // because there are additional vertex data that gets created through this which are required by the Material
    ...
    {
        if (data.Model is null || data.RenderMesh is null || data.Material is null)
        {
            // Shouldn't be null...
            return;
        }
    ...
    private void UpdateMaterial(RenderMesh renderMesh, MaterialPass materialPass, MaterialInstance? modelMaterialInstance)
    {
        renderMesh.MaterialPass = materialPass;

        renderMesh.IsShadowCaster = false;
        if (modelMaterialInstance is not null)
        {
    ...
    {
        if (data.RenderMesh is null && data.Model is not null)
        {
            ...
            {
                UpdateMaterial(renderMesh, material.Passes[0], model.Materials.GetItemOrNull(0));
            }
